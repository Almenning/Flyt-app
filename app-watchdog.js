...
function restoreOwnedView(view){if(view==='home'&&window.FlytHomeUI?.render){queueMicrotask(()=>window.FlytHomeUI.render());return}if(view==='tasks'&&window.FlytTasksUI?.render){queueMicrotask(()=>{window.FlytTasksUI.render();window.FlytPlannedUI?.paint?.();window.FlytRecurrenceUI?.render?.();});return}if(view==='us'&&window.FlytOss?.render){queueMicrotask(()=>window.FlytOss.render({refresh:false}));return}if(view==='seen'&&window.FlytSeenUI?.render){queueMicrotask(()=>window.FlytSeenUI.render())}}
...
function loadPlanned(){loadScript('./planned-ui.js?v=20260824-1457','flyt-planned')}
function loadDayCompleted(){loadScript('./tasks-day-completed-ui.js?v=20260824-1350','flyt-day-completed')}
function loadModal(){loadScript('./modal-ui.js?v=20260824-1414','flyt-modal')}
function loadCustomCategories(){loadScript('./custom-categories-ui.js?v=20260824-1548','flyt-custom-categories')}
...
document.addEventListener('click',async e=>{
 ...
 if(custom){e.preventDefault();e.stopImmediatePropagation();if(window.FlytCustomCategories?.openGeneral){await window.FlytCustomCategories.openGeneral();return}const name=await askPrompt({ey:'Oppsett',title:'Legg til egendefinert oppgave',label:'Navn på oppgaven',placeholder:'F.eks. Rydde boden',ok:'Legg til'});if(!name?.trim())return;const b=bridge(),s=b?.getState?.();if(!s)return;const x={id:'custom_'+Date.now(),cat:'Egendefinert',name:name.trim(),freq:1,pts:30,type:'flex',kind:'house'};saveState({...s,custom:[...(s.custom||[]),x],tasks:[...(s.tasks||[]),{...x,owner:'Begge'}]});window.FlytTasksUI?.openSetup?.(1);setTimeout(()=>document.querySelector('#setupBody')?.scrollTo({top:document.querySelector('#setupBody')?.scrollHeight||0,behavior:'smooth'}),40);return}
 ...
 const delPlanned=e.target.closest('[data-planned-delete]');
 if(delPlanned){e.preventDefault();e.stopImmediatePropagation();const b=bridge(),s=b?.getState?.(),items=Array.isArray(s?.plannedTasks)?s.plannedTasks:[],x=items.find(x=>String(x.id)===String(delPlanned.dataset.plannedDelete));if(!s||!x)return;if(x.done){b.toast?.('Angre registreringen før du sletter');return}const yes=await askConfirm({ey:'Ekstraoppgave',title:'Slette ekstraoppgaven?',text:'Oppgaven fjernes fra planen.',ok:'Slett'});if(!yes)return;saveState({...s,plannedTasks:items.filter(x=>String(x.id)!==String(delPlanned.dataset.plannedDelete))});window.FlytPlannedUI?.paint?.();return}
...
window.addEventListener('DOMContentLoaded',()=>{loadResponsive();loadHome();loadModal();loadPlanned();loadDayCompleted();loadCustomCategories();loadRecurrence();loadBeta();keepGuardAlive();setTimeout(rescue,5000)});
