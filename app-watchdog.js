...
function restoreOwnedView(view){if(view==='home'&&window.FlytHomeUI?.render){queueMicrotask(()=>window.FlytHomeUI.render());return}if(view==='tasks'&&window.FlytTasksUI?.render){queueMicrotask(()=>{window.FlytTasksUI.render();window.FlytPlannedUI?.paint?.()});return}if(view==='us'&&window.FlytOss?.render){queueMicrotask(()=>window.FlytOss.render({refresh:false}))}}
...
function loadPlanned(){if(document.querySelector('script[data-flyt-planned]'))return;const s=document.createElement('script');s.src='./planned-ui.js?v=20260824-0945';s.defer=true;s.dataset.flytPlanned='1';document.head.appendChild(s)}
window.addEventListener('DOMContentLoaded',()=>{loadPlanned();keepGuardAlive();setTimeout(rescue,5000)});
