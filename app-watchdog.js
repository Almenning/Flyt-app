...
function restoreOwnedView(view){if(view==='home'&&window.FlytHomeUI?.render){queueMicrotask(()=>window.FlytHomeUI.render());return}if(view==='tasks'&&window.FlytTasksUI?.render){queueMicrotask(()=>window.FlytTasksUI.render());return}if(view==='us'&&window.FlytOss?.render){queueMicrotask(()=>window.FlytOss.render({refresh:false}))}}
