...
function loadRecurrence(){if(window.FlytRecurrenceUI)return;loadScript('./recurrence-ui.js?v=20260825-0635','flyt-recurrence')}
...
window.addEventListener('DOMContentLoaded',()=>{loadLegacyCleanup();loadResponsive();loadHome();loadModal();loadPlanned();loadDayCompleted();loadCustomCategories();loadRecurrence();loadBeta();loadRewardsEditGuard();loadRewardsSummary();loadRewardsUI();loadSetupNavGuard();loadTaskActionsGuard();keepGuardAlive();setTimeout(rescue,5000)});
