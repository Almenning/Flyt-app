...
function ensureHomeOwnership(){const b=window.FlytBridge,c=document.querySelector('#content');if(b?.getState?.()?.view!=='home')return false;if(window.FlytHomeUI?.version!=='20260827-2005')return false;if(c?.dataset.flytOwner==='home')return true;window.FlytHomeUI.claim?.({resetScroll:false})||window.FlytHomeUI.render?.({resetScroll:false});return c?.dataset.flytOwner==='home'}
...
window.addEventListener('DOMContentLoaded',()=>{loadStartupHydration();loadBuyerPolish();loadResponsive();loadHome();loadModal();loadSeen();loadSeenRequestAlert();loadRewardsEditGuard();loadPlanned();loadDayCompleted();loadCustomCategories();loadRecurrence();loadBeta();loadRewardsSummary();loadRewardsUI();loadQuickTemptation();loadSetupV2();loadHistory();keepGuardAlive();queueMicrotask(ensureHomeOwnership);setTimeout(ensureHomeOwnership,150);setTimeout(rescue,5000)});
window.addEventListener('pageshow',()=>{loadHome();ensureHomeOwnership()});
