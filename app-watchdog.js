...
function ensureHomeOwnership(){const b=window.FlytBridge,c=document.querySelector('#content');if(b?.getState?.()?.view!=='home')return true;if(window.FlytHomeUI?.version!=='20260830-1530'){loadHome();return false}if(homeMarkupIsModern(c)){window.FlytNudgeUI?.augment?.();return true}window.FlytHomeUI.render?.({resetScroll:false});window.FlytNudgeUI?.augment?.();return homeMarkupIsModern(c)}
...
function loadCoupleInvitations(){if(window.FlytCoupleInvitations?.version==='20260831-1200')return;loadScript('./couple-invitation-ui.js?v=20260831-1200','flyt-couple-invitations-1200')}
function loadSetupV2(){if(window.FlytSetupV2?.version==='20260828-2115')return;loadScript('./setup-v2.js?v=20260828-2115','flyt-setup-v2-2115')}
function loadHome(){if(window.FlytHomeUI?.version==='20260830-1530'){ensureHomeOwnership();return}loadScript('./home-ui.js?v=20260830-1530','flyt-home-current-1530',()=>queueMicrotask(ensureHomeOwnership))}
function loadNudge(){if(window.FlytNudgeUI?.version==='20260831-1200'){window.FlytNudgeUI.augment?.();return}loadScript('./nudge-ui.js?v=20260831-1200','flyt-nudge-1200',()=>queueMicrotask(()=>window.FlytNudgeUI?.augment?.()))}
...
window.addEventListener('DOMContentLoaded',()=>{loadStartupHydration();loadBuyerPolish();loadAccount();loadResponsive();loadCoupleCore();loadHome();loadNudge();loadModal();loadSeen();loadSeenRequestAlert();loadRewardsEditGuard();loadPlanned();loadDayCompleted();loadCustomCategories();loadRecurrence();loadBeta();loadRewardsSummary();loadRewardsUI();loadCoupleInvitations();loadQuickTemptation();loadSetupV2();loadHistory();keepGuardAlive();guardHomeStartup();setTimeout(rescue,5000)});
window.addEventListener('pageshow',()=>{loadBuyerPolish();loadAccount();loadCoupleCore();loadHome();loadNudge();loadCoupleInvitations();window.FlytAccountUI?.checkConsent?.();guardHomeStartup()});
