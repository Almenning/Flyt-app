...
function loadSeen(){if(window.FlytSeenUI?.version==='20260830-1600')return;loadScript('./seen-ui.js?v=20260830-1600','flyt-seen-current-1600')}
...
function loadNudge(){if(window.FlytNudgeUI?.version==='20260830-1530'){window.FlytNudgeUI.augment?.();return}loadScript('./nudge-ui.js?v=20260830-1530','flyt-nudge-1530',()=>queueMicrotask(()=>window.FlytNudgeUI?.augment?.()))}
...
window.addEventListener('DOMContentLoaded',()=>{loadStartupHydration();loadBuyerPolish();loadAccount();loadResponsive();loadHome();loadNudge();loadModal();loadSeen();loadSeenRequestAlert();loadRewardsEditGuard();loadPlanned();loadDayCompleted();loadCustomCategories();loadRecurrence();loadBeta();loadRewardsSummary();loadRewardsUI();loadQuickTemptation();loadSetupV2();loadHistory();keepGuardAlive();guardHomeStartup();setTimeout(rescue,5000)});
window.addEventListener('pageshow',()=>{loadBuyerPolish();loadAccount();loadHome();loadNudge();window.FlytAccountUI?.checkConsent?.();guardHomeStartup()});
