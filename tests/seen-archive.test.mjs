...
assert.match(seen, /const VERSION='20260902-1700'/, 'Sett must expose the current communication version');
...
assert.match(seen, /const STALE_REQUEST_MS=14\*DAY_MS/, 'Unanswered requests must leave the active inbox after 14 days');
assert.match(seen, /function requestExpired\(/, 'Sett must identify stale requests');
...
assert.match(seen, /data-seen-mode="now"/, 'Sett must provide a dedicated Now view');
assert.match(seen, /data-seen-mode="archive"/, 'Sett must provide a dedicated Archive view');
assert.match(seen, /Trenger svar/, 'Sett must separate items waiting for a response');
assert.match(seen, /<div class="ey">Avtalt<\/div>/, 'Sett must separate accepted agreements');
assert.match(seen, /Ferdige ting vises bare når et lite takk fortsatt gjenstår/, 'Completed items must only remain visible when acknowledgement is pending');
...
assert.match(seen, /Ferdige og utløpte ting ligger her i opptil 90 dager/, 'Archive retention must be explained in the UI');
...
assert.match(watchdog, /seen-ui\.js\?v=20260902-1700/, 'Watchdog must cache-bust the current Sett UI');
assert.match(watchdog, /seen-request-alert-ui\.js\?v=20260902-1700/, 'Watchdog must cache-bust the current Sett alert UI');
