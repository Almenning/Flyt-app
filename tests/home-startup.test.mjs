...
assert.match(home, /const VERSION='20260903-1200'/, 'Home must expose the current startup version');
...
assert.match(home, /data-home-next/, 'Home must expose the next unfinished chores');
assert.match(home, /data-home-week/, 'Home must expose compact weekly progress');
...
assert.match(home, /\$\{weekMarkup\(s,week\)\}<div id="homeNudgeMount"/, 'the contextual nudge must follow weekly progress');
...
assert.match(watchdog, /FlytHomeUI\?\.version!=='20260903-1200'/, 'Watchdog must target the current Home version');
assert.match(watchdog, /home-ui\.js\?v=20260903-1200/, 'Watchdog must cache-bust the current Home module');
assert.match(watchdog, /nudge-ui\.js\?v=20260903-1200/, 'Watchdog must load the contextual nudge module');
