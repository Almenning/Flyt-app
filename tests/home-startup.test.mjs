...
assert.match(home, /const VERSION='20260903-1800'/, 'Home must expose the current startup version');
...
assert.match(home, /\$\{dailyStatusMarkup\(s\)\}\$\{dailyGoalMarkup\(plan\)\}<div id="homeNudgeMount"/, 'Home must show daily status, daily goal, then at most one contextual nudge');
...
assert.match(watchdog, /FlytHomeUI\?\.version!=='20260903-1800'/, 'Watchdog must target the current Home version');
assert.match(watchdog, /home-ui\.js\?v=20260903-1800/, 'Watchdog must cache-bust the current Home module');
