...
assert.match(index, /hverdagsoss_sw_reset_v67/, 'Gamle service workers må avregistreres én gang før appskriptene lastes');
assert.match(index, /registration=>registration\.unregister\(\)/, 'Den fastlåste service workeren må faktisk avregistreres');
assert.match(serviceWorker, /const CACHE='flyt-v68'/, 'Offline-cachen må fornyes når produktlogikken endres');
