...
const recurrenceVersion=recurrence.match(/const VERSION='([^']+)'/)?.[1];
assert.ok(recurrenceVersion,'Gjøre must expose an explicit UI version');
...
assert.ok(watchdog.includes(`recurrence-ui.js?v=${recurrenceVersion}`),'the watchdog must load the current task UI version');
assert.ok(index.includes(`recurrence-ui.js?v=${recurrenceVersion}`),'the initial task UI load must use the current task UI version');
