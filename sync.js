...
async function pull(force=false){if(!ctx?.household||!hydrated)return false;if((dirty||saving)&&!force)return false;try{await loadContext();applyRemote();updateChrome();return true}catch(e){return false}}
...
async function push(){
  ...
  try{
    ...
    return true;
  }catch(e){console.error('HverdagsOss save failed:',e);dirty=true;syncError=true;return false}finally{saving=false;updateChrome()}
}
