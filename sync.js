...
function inviteScreen(code,expiresAt=ctx?.household?.invite_expires_at,after='app'){const expiry=formatInviteExpiry(expiresAt),expired=expiresAt&&new Date(expiresAt).getTime()<=Date.now(),shown=expired?'Utløpt':code||'Ingen aktiv kode';showGate(shell(`<div class="ey" style="margin-top:16px">Inviter partner</div><h2 style="margin:8px 0">Del HverdagsOss</h2><p class="sub">Koden kan brukes én gang og utløper automatisk etter sju dager.</p><div class="card hero" style="text-align:center"><div class="ey">Invitasjonskode</div><div style="font-size:30px;font-weight:900;letter-spacing:.1em;margin:10px 0;overflow-wrap:anywhere">${esc(shown)}</div>${expiry?`<div class="taskmeta">${expired?'Utløpt':'Gyldig til'} ${esc(expiry)}</div>`:''}</div><button id="rotateInvite" class="secondary full">Lag ny invitasjonskode</button><button id="continueSetup" class="primary full" style="margin-top:10px">Fortsett til HverdagsOss</button><p id="betaStatus" class="sub">En ny kode gjør den gamle ugyldig med én gang.</p>`));$('#rotateInvite').onclick=()=>rotateInviteCode(after);$('#continueSetup').onclick=()=>{hydrated=true;if(after==='setup')openHouseSetup(false);else showApp()}}
