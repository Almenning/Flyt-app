...
function moodPersonMarkup({name,status,mine=false}){const info=levelInfo(statusLevel(status)),meta=status?age(dailyStamp(status)):'Ikke registrert';return `<div class="homeMoodPerson ${mine?'isMine':''}" ${mine?'data-home-status-edit role="button" tabindex="0" aria-label="Endre din dagsform"':'data-home-partner-status'}><span class="homeMoodAvatar" aria-hidden="true">${esc(String(name||'?').trim().charAt(0).toUpperCase())}</span><div class="homeMoodCopy"><span>${mine?'Deg':esc(name)}</span><strong>${esc(info?.label||'Ikke registrert')}</strong>${moodDots(status)}<small>${esc(meta)}</small></div>${mine?'<span class="homeMoodEdit">Endre</span>':''}</div>`}
...
function dailyStatusMarkup(s){const status=freshDaily(myStatus(s)),partnerStatus=hasPartner()?freshDaily(partnerDailyStatus(s)):null;return `<section class="homeSurface homeMoodCard" data-home-status-card data-home-couple-status><div class="homeSectionTitle"><span class="homeSectionIcon" aria-hidden="true">☺</span><div><h2>Dagsform</h2><p>Hvordan har dere det i dag?</p></div></div><div class="homeMoodGrid">${moodPersonMarkup({name:currentName(s),status,mine:true})}${hasPartner()?moodPersonMarkup({name:partnerName(s),status:partnerStatus}):`<div class="homeMoodPerson isEmpty"><span class="homeMoodAvatar" aria-hidden="true">♡</span><div class="homeMoodCopy"><span>Partner</span><strong>Ikke koblet til</strong>${moodDots(null)}</div></div>`}</div>${statusEditorOpen?(statusDraft||startStatusDraft(s),statusEditorMarkup(s)):''}</section>`}
