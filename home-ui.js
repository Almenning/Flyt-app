...
.homeChallengeCard{padding:10px 12px}.homeChallengeEmpty,.homeChallengeActive{gap:2px;margin-top:7px;padding:8px 9px;border-radius:14px}.homeChallengeEmpty strong,.homeChallengeActive strong{font-size:16px}.homeChallengeEmpty span,.homeChallengeActive>span,.homeChallengeReward{font-size:11px}.homeChallengeProgress{height:5px;margin-top:4px}.homeChallengeActions{gap:7px;margin-top:7px}.homeChallengeActions .primary,.homeChallengeActions .secondary{min-height:40px;padding:7px;font-size:11px}.homeChallengeCard .homeTextAction{margin-top:3px;font-size:11px}
...
function render({resetScroll=false}={}){
  ...
  c.innerHTML=`<header class="homeHeader"><div class="ey">Hjem</div><h1 class="title">${greeting()}, ${esc(name)}</h1><p class="sub">Et raskt overblikk over dagen deres</p></header>${dailyGoalMarkup(plan)}${dailyStatusMarkup(s)}${homeChallengeMarkup(s)}<div id="homeNudgeMount" ${first?'data-first-win-active="1"':''} aria-live="polite">${first}</div>${statusSheetMarkup(s)}`;
