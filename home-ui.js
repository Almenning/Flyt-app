...
function statusShortcutMarkup(previous){const info=levelInfo(statusLevel(previous));if(!previous||!info)return'';return `<div class="homeMoodShortcut"><small>I går: ${esc(info.label)}</small><button type="button" class="small" data-home-status-same="1">Samme i dag</button></div>`}
function moodPersonMarkup({name,status,mine=false,previous=null}){const info=levelInfo(statusLevel(status)),missing=mine&&!status,meta=mine&&status?'Oppdatert i dag':status?age(dailyStamp(status)):'Ikke registrert',partnerAttributes=mine?'':'data-home-partner-status';return `<div class="homeMoodPerson ${mine?'isMine':''} ${missing?'isMissing':''}" ${partnerAttributes}><span class="homeMoodAvatar" aria-hidden="true">${esc(String(name||'?').trim().charAt(0).toUpperCase())}</span><div class="homeMoodCopy"><span>${mine?'Deg':esc(name)}</span><strong>${esc(info?.label||'Ikke registrert')}</strong>${moodDots(status)}<small>${esc(meta)}</small></div>${mine&&!missing?'<button type="button" class="homeMoodEdit" data-home-status-edit aria-label="Endre din dagsform">Endre status</button>':''}${missing?`${quickStatusChoices()}${statusShortcutMarkup(previous)}`:''}</div>`}
...
function dailyStatusMarkup(s){const rawMyStatus=myStatus(s),status=freshDaily(rawMyStatus),previous=previousDaily(rawMyStatus),partnerStatus=hasPartner()?freshDaily(partnerDailyStatus(s)):null;return `<section class="homeSurface homeMoodCard" data-home-status-card data-home-couple-status><div class="homeSectionTitle"><span class="homeSectionIcon" aria-hidden="true">☺</span><div><h2>Dagsform</h2><p>Hvordan har dere det i dag?</p></div></div><div class="homeMoodGrid">${moodPersonMarkup({name:currentName(s),status,mine:true,previous})}${hasPartner()?moodPersonMarkup({name:partnerName(s),status:partnerStatus}):`<div class="homeMoodPerson isEmpty"><span class="homeMoodAvatar" aria-hidden="true">♡</span><div class="homeMoodCopy"><span>Partner</span><strong>Ikke koblet til</strong>${moodDots(null)}</div></div>`}</div></section>`}
function statusSheetMarkup(s){if(!statusEditorOpen)return'';statusDraft||startStatusDraft(s);return `<div id="homeStatusSheet" class="homeStatusSheetLayer" role="presentation"><section class="homeStatusSheet" role="dialog" aria-modal="true" aria-labelledby="homeStatusSheetTitle"><div class="homeStatusSheetHandle" aria-hidden="true"></div><div class="homeStatusSheetHead"><div><div class="ey">Dagsform</div><h2 id="homeStatusSheetTitle">Hvordan har du det i dag?</h2></div><button type="button" class="homeStatusSheetClose" data-home-status-cancel aria-label="Lukk">×</button></div>${statusEditorMarkup(s)}</section></div>`}
...
function ensureStyles(){if($('#flytHomeStyles')||typeof document.createElement!=='function')return;const style=document.createElement('style');style.id='flytHomeStyles';style.textContent=`
  ...
  .homeHeader{margin:0 2px 14px}.homeHeader .ey{color:#b85e43;letter-spacing:.17em}.homeHeader .title{margin:5px 0 5px;font-weight:500;color:#392822}.homeHeader .sub{margin:0;color:#806b61}
  .homeSurface{margin:10px 0;padding:14px;border:1px solid #eadbd2;border-radius:22px;background:#fffdf9;box-shadow:0 7px 20px #79503a0b}
  ...
  .homeMoodCard{padding:16px}.homeMoodGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(245px,1fr));gap:10px;margin-top:11px}.homeMoodPerson{position:relative;display:flex;align-items:center;gap:9px;min-width:0;padding:10px;border:1px solid #eee0d6;border-radius:17px;background:#fffaf6}.homeMoodPerson.isMissing{display:flex;flex-wrap:wrap}.homeMoodAvatar{width:40px;height:40px;display:grid;place-items:center;flex:0 0 40px;border-radius:50%;background:#f6d9c6;color:#7a4a39;font:600 20px/1 Georgia,serif}.homeMoodPerson:nth-child(2) .homeMoodAvatar{background:#f7e4d4}.homeMoodCopy{min-width:0;display:grid;flex:1}.homeMoodCopy>span{font-size:13px;color:#78635a}.homeMoodCopy strong{font:500 19px/1.2 Georgia,serif;overflow-wrap:anywhere}.homeMoodCopy small{margin-top:4px;color:#98857c;font-size:11px;overflow-wrap:anywhere}.homeMoodDots{display:flex;gap:5px;margin-top:7px}.homeMoodDots span{width:9px;height:9px;border-radius:50%;background:#edddd1}.homeMoodDots span.isFilled{background:#df9a75}.homeMoodDots span.isSelected{box-shadow:0 0 0 2px #fff,0 0 0 3px #c76e4e}.homeMoodEdit{flex:0 0 auto;align-self:flex-start;margin:1px 0 0;border:0;background:transparent;color:#a85b44;font:700 12px/1.2 inherit;text-decoration:underline;text-underline-offset:3px}.homeMoodEdit:focus-visible{outline:3px solid #d9775744;outline-offset:2px;border-radius:4px}.homeMoodQuickChoices{display:grid;grid-template-columns:repeat(5,1fr);gap:4px;flex-basis:100%;margin-top:9px}.homeMoodQuickChoices button{min-width:0;border:0;border-radius:9px;background:#f8e9df;color:#806056;padding:6px 1px;font:inherit}.homeMoodQuickChoices button span{display:block;width:10px;height:10px;margin:0 auto 3px;border-radius:50%;background:#d77b52}.homeMoodQuickChoices button:nth-child(1) span{background:#c98668}.homeMoodQuickChoices button:nth-child(2) span{background:#da9b75}.homeMoodQuickChoices button:nth-child(3) span{background:#e7b38b}.homeMoodQuickChoices button:nth-child(4) span{background:#dc8a61}.homeMoodQuickChoices button:nth-child(5) span{background:#c96e4d}.homeMoodQuickChoices small{display:block;font-size:11px;line-height:1.1;white-space:normal}.homeMoodShortcut{display:flex;flex-wrap:wrap;align-items:center;gap:4px;flex-basis:100%;margin-top:8px;color:#806b61}.homeMoodShortcut>small{flex:1 0 100%;font-size:12px}.homeMoodShortcut .small{padding:5px 7px;font-size:12px}
  .homeStatusSheetLayer{position:fixed;inset:0;z-index:350;display:flex;align-items:flex-end;background:#3a211b85;backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px)}.homeStatusSheet{width:min(100%,430px);max-height:86dvh;overflow:auto;padding:10px 16px max(20px,env(safe-area-inset-bottom));border-radius:26px 26px 0 0;background:linear-gradient(#fffaf7,#fff6f1);box-shadow:0 -20px 54px #3a211b30}.homeStatusSheetHandle{width:38px;height:5px;margin:3px auto 12px;border-radius:999px;background:#e6d3cb}.homeStatusSheetHead{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.homeStatusSheetHead h2{margin:5px 0 0;font:600 26px/1.12 Georgia,serif;color:#3d2b25}.homeStatusSheetClose{width:38px;height:38px;border:1px solid #eadbd2;border-radius:50%;background:#fffaf7;color:#9a553f;font-size:25px;line-height:1}.homeMoodEditor{margin-top:16px;padding-top:0;border-top:0}.homeMoodChoices{display:grid;grid-template-columns:repeat(5,1fr);gap:5px;margin-top:9px}.homeMoodChoices button{min-width:0;padding:9px 3px;border:1px solid #eadbd2;border-radius:12px;background:#fffaf7;color:#705d54;font-size:11px;line-height:1.2}.homeMoodChoices button span{display:block;width:12px;height:12px;margin:0 auto 5px;border-radius:50%;background:#edddd1}.homeMoodChoices button.on{border-color:#d47454;background:#fff0e8;color:#874632;box-shadow:0 0 0 2px #d474541a}.homeMoodChoices button.on span{background:#d47454}.homeMoodNeedsTitle{margin-top:15px;font-weight:800}.homeMoodNeeds{margin-top:8px}.homeMoodNotify{display:flex;gap:9px;margin-top:12px;padding:10px;border:1px solid #eadbd2;border-radius:14px;background:#fffaf7}.homeMoodNotify input{margin-top:3px;accent-color:#cf6f4f}.homeMoodNotify span{display:grid}.homeMoodNotify small{color:#816e65;margin-top:2px}.homeMoodError{color:#a63c31;font-weight:750}.homeMoodEditorActions{display:grid;grid-template-columns:1fr 1.35fr;gap:9px;margin-top:13px}
  ...
  .homeChallengeCard{padding:13px 14px}.homeChallengeCard .homeSectionTitle p{max-width:260px}.homeChallengeEmpty,.homeChallengeActive{display:grid;gap:3px;margin:10px 0 0;padding:10px 11px;border:1px solid #eee0d6;border-radius:16px;background:#fffaf6}.homeChallengeEmpty strong,.homeChallengeActive strong{font:600 17px/1.2 Georgia,serif;color:#453029}.homeChallengeEmpty span,.homeChallengeActive>span{color:#806b61;font-size:12px;line-height:1.35}.homeChallengeReward{margin-top:2px;color:#a85b44;font-size:12px;font-weight:750}.homeChallengeProgress{height:6px;overflow:hidden;margin-top:6px;border-radius:999px;background:#f1e3dc}.homeChallengeProgress i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#d77b52,#edb18a)}.homeChallengeActions{display:grid;grid-template-columns:1.2fr 1fr;gap:9px;margin-top:9px}.homeChallengeActions .primary,.homeChallengeActions .secondary{min-height:42px;margin:0;padding:8px;font-size:12px}.homeChallengeCard .homeTextAction{margin:6px 0 0 auto}
...
.homeGoalCard{padding:11px 14px 9px}
...
.homeGoalCard .homeGoalGrid{grid-template-columns:minmax(100px,.9fr) minmax(124px,1.1fr);gap:10px;margin-top:7px}
.homeGoalCard .homeProgressRing{width:min(28vw,110px)}
...
.homeGoalCard .homeGoalSummary{display:grid;gap:4px;align-content:center;padding:3px 0}.homeGoalCard .homeGoalSummary>strong{font:600 23px/1 Georgia,serif;color:#49342c}.homeGoalCard .homeGoalSummary>span{color:#b9684b;font-size:13px;line-height:1.3}.homeGoalCard .homeGoalSummary .homeTextAction{justify-self:start;margin:3px 0 0;padding:2px 0;font-size:12px}
...
/* Bevisst tett Hjem: Dagsform beholder mest vekt, resten bruker kortere rytme. */
  .homeHeader{margin-bottom:8px}.homeHeader .title{margin:3px 0 0}.homeSurface{margin:7px 0;padding:11px 12px;border-radius:19px}.homeSectionTitle{gap:8px}.homeSectionTitle h2,.homeSectionTitle h2:first-child{font-size:20px}.homeSectionTitle p{margin-top:2px;font-size:13px}.homeSectionIcon{width:29px;height:29px;flex-basis:29px;font-size:17px}
  .homeGoalCard{padding:9px 12px 8px;border-color:#e8cbbd;background:linear-gradient(145deg,#fffdf9,#fff8f4);box-shadow:0 8px 22px #79503a0d}.homeGoalCard .homeGoalGrid{grid-template-columns:minmax(84px,.78fr) minmax(132px,1.22fr);gap:8px;margin-top:5px}.homeGoalCard .homeProgressRing{width:min(24vw,92px)}.homeGoalCard .homeProgressRing:after{inset:9px}.homeGoalCard .homeProgressRing strong{font-size:18px}.homeGoalCard .homeGoalSummary{gap:2px;padding:1px 0}.homeGoalCard .homeGoalSummary>strong{font-size:20px}.homeGoalCard .homeGoalSummary>span{color:#806b61;font-size:12px}.homeGoalCard .homeGoalSummary .homeTextAction{margin-top:1px;font-size:11px}.homeGoalCard .homeTextAction{font-size:11px}
.homeMoodCard{padding:12px}.homeMoodGrid{gap:8px;margin-top:8px}.homeMoodPerson{gap:7px;padding:8px;border-radius:14px}.homeMoodAvatar{width:34px;height:34px;flex-basis:34px;font-size:17px}.homeMoodCopy>span{font-size:12px}.homeMoodCopy strong{font-size:17px}.homeMoodCopy small{margin-top:2px}.homeMoodDots{gap:4px;margin-top:5px}.homeMoodDots span{width:8px;height:8px}.homeMoodEdit{right:7px;top:5px}.homeMoodQuickChoices{margin-top:6px}.homeMoodQuickChoices button{padding:5px 1px}.homeMoodShortcut{margin-top:6px}
  .homeChallengeCard{padding:10px 12px}.homeChallengeCard:not([data-home-challenge-id]){border-color:#f0e5de;background:#fffcfa;box-shadow:none}.homeChallengeCompact{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:4px 8px;align-items:center}.homeChallengeCompact .homeSectionTitle h2{font-size:18px}.homeChallengeEmpty{margin:0;padding:0;border:0;background:transparent;color:#98857c;font-size:12px}.homeChallengeActive{gap:2px;margin-top:7px;padding:8px 9px;border-radius:14px}.homeChallengeActive strong{font-size:16px}.homeChallengeActive>span,.homeChallengeReward{font-size:11px}.homeChallengeProgress{height:5px;margin-top:4px}.homeChallengeActions{grid-column:1/-1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:3px}.homeChallengeTextAction{min-height:42px;padding:7px 8px;border:1px solid #eadbd2;border-radius:11px;background:transparent;color:#855846;font:700 12px/1.2 inherit}.homeChallengeTextAction:focus-visible{outline:3px solid #d9775744;outline-offset:2px}.homeChallengeCard .homeTextAction{margin-top:3px;font-size:11px}
@media(max-width:390px){.homeSurface{padding:10px 11px}.homeGoalCard .homeProgressRing{width:86px}.homeMoodPerson{padding:7px 6px}.homeMoodAvatar{width:31px;height:31px;flex-basis:31px}.homeMoodCopy strong{font-size:16px}}
`;document.head.appendChild(style)}
...
function goalEncouragement(plan){if(!plan.total)return'Ingen planlagte gjøremål';if(!plan.remaining)return'Dagens mål er nådd';if(plan.pct>=70)return'Nesten i mål';if(plan.pct>=40)return'Godt i rute';if(plan.done)return'God start';return'I gang'}
...
function homeChallengeMarkup(s){
  ...
  if(!challenge)return `<section class="homeSurface homeChallengeCard" data-home-challenge-card><div class="homeChallengeCompact"><div class="homeSectionTitle"><span class="homeSectionIcon" aria-hidden="true">◇</span><h2>Utfordring</h2></div><span class="homeChallengeEmpty">Ingen aktiv</span><div class="homeChallengeActions"><button type="button" class="homeChallengeTextAction" data-home-challenge-create="1">Gi utfordring</button><button type="button" class="homeChallengeTextAction" data-home-challenge-request="1">Be om en</button></div></div></section>`;
