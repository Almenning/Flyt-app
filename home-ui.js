...
function dailyStatusMarkup(s){const raw=myStatus(s),status=freshDaily(raw),needs=cleanNeeds(status?.needs);if(statusEditorOpen){if(!statusDraft)startStatusDraft(s);return `<section class="card" data-home-status-card style="margin:14px 0;border-color:#e6cfc4;background:linear-gradient(145deg,#fffdfb,#fff1e9)"><div class="ey">Dagsform i dag</div>${statusEditorMarkup(s)}</section>`}if(!status)return `<section class="card" data-home-status-card style="margin:14px 0;border-color:#e6cfc4;background:linear-gradient(145deg,#fffdfb,#fff4ed)"><div class="row"><div class="grow"><div class="ey">Dagsform i dag</div><strong style="display:block;font:600 21px/1.2 Georgia;margin-top:6px">Hvordan er dagsformen din?</strong><div class="taskmeta" style="margin-top:6px">Ikke oppdatert i dag</div></div><button type="button" class="small" data-home-status-edit>Sjekk inn</button></div></section>`;const label=dailyApi()?.capacityLabel?.(status.capacity)||LABEL[status.capacity]||'Oppdatert';return `<section class="card" data-home-status-card style="margin:14px 0;border-color:#e0dac6;background:linear-gradient(145deg,#fffdfb,#f7f5eb)"><div class="row"><div class="grow"><div class="ey">Dagsform i dag</div><strong style="display:block;font:600 21px/1.2 Georgia;margin-top:6px">Du har ${esc(label.toLowerCase())}</strong><div class="taskmeta" style="margin-top:5px">${age(dailyStamp(status))}</div></div><button type="button" class="small" data-home-status-edit>Endre</button></div>${needs.length?`<div style="display:flex;flex-wrap:wrap;gap:7px;margin-top:12px">${needs.map(value=>`<span class="tag">${esc(dailyApi()?.needLabel?.(value)||value)}</span>`).join('')}</div>`:''}</section>`}
...
function render({resetScroll=false}={}){
  ...
  c.innerHTML=`<div class="ey">Hjem · Dere i dag</div><h1 class="title">Dere i dag</h1><p class="sub">${greeting()}, ${esc(name)}. ${intro}</p>${dailyStatusMarkup(s)}<div id="homeNudgeMount" ${first?'data-first-win-active="1"':''} aria-live="polite">${first}</div>${tabs()}${hero}${stats}${partnerCard(s)}${tail}`;
