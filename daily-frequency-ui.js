...
function install(){tuneSetup();const root=document.querySelector('#flytSetupV2');if(root&&!observer){observer=new MutationObserver(()=>queueMicrotask(tuneSetup));observer.observe(root,{childList:true,subtree:true,attributes:true,attributeFilter:['class']})}document.addEventListener('change',e=>{if(e.target.matches?.('select[data-v2-type],input[data-v2-freq]'))setTimeout(tuneSetup,0)},true);window.addEventListener('pageshow',tuneSetup)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
window.FlytDailyFrequencyUI={tuneSetup,version:'20260825-2352'};
})();