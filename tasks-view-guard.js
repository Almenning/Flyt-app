(()=>{
'use strict';
let restoring=false,observer=null;
...
function install(){window.addEventListener('click',intercept,true);const c=content();if(c){observer=new MutationObserver(()=>queueMicrotask(()=>restore({resetScroll:false})));observer.observe(c,{childList:true,subtree:true})}[0,80,180,400,900,1800,3000].forEach(ms=>setTimeout(()=>restore({resetScroll:false}),ms));window.addEventListener('pageshow',()=>setTimeout(()=>restore({resetScroll:false}),0));document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')setTimeout(()=>restore({resetScroll:false}),0)})}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
window.FlytTasksViewGuard={restore,openTasks,version:'20260828-2208'};
})();