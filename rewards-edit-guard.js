...
function ensureSeenUi(){if(window.FlytDialog?.openReward||seenLoading)return;seenLoading=true;const s=document.createElement('script');s.src='./seen-ui.js?v=20260825-0702';s.defer=true;s.dataset.flytSeenReward='1';s.onload=()=>{seenLoading=false};s.onerror=()=>{seenLoading=false};document.head.appendChild(s)}
...
const obs=new MutationObserver(()=>{if(draft)restore()});window.addEventListener('DOMContentLoaded',()=>{ensureSeenUi();const c=document.querySelector('#content');if(c)obs.observe(c,{childList:true,subtree:true})});let tries=0;const timer=setInterval(()=>{const c=document.querySelector('#content');if(c){obs.observe(c,{childList:true,subtree:true});clearInterval(timer)}else if(++tries>60)clearInterval(timer)},100);
window.FlytRewardsEditGuard={restore,openReward,version:'20260825-0708'};
})();